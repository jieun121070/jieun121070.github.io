...
이처럼 정규화 상수 $Z_\theta$를 계산하는 것이 현실적으로 불가능하기 때문에 사용하는 방법이 바로 **Score matching**입니다. Score matching은 실제 분포 $p_\text{data}(x)$와 모델 $p_\theta(x)$의 을 스코어 함수를 최대한 가깝게 만드는 것을 목표로 하는 학습 방법입니다. 이에 따라 두 스코어 함수 사이의 차이를 최소화하는 아래와 같은 loss function을 사용합니다. 여기에서 스코어 함수는 log-likelihood의 $\theta$에 대한 gradient가 아니라 **$x$에 대한 gradient**로, $s(x)=\nabla_x \log p_\theta(x)$입니다.
