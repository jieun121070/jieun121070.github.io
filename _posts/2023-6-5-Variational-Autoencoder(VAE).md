...
VAE는 생성 모델의 일종으로 Autoencoder와는 목적이 다릅니다. **VAE는 실제 데이터 분포 $p_\text{data}(x)$를 잘 학습해서 파라미터 $\theta$로 데이터 분포를 흉내 내는 확률 모델 $p_\theta(x)$를 구하고자 합니다.** 이를 위해 사전 확률 $p_\theta(z)$를 정의하고, encoder $q_\theta(z \vert x)$를 통해 input $x$를 분포로 매핑합니다. 앞서 Autoencoder의 encoder가 input을 고정된 vector로 매핑했던 것과는 다릅니다. 그리고 진짜처럼 보이는 **새로운 데이터를 생성할 수 있는 decoder $p_\theta(x \vert z)$**를 학습시킵니다.
...
하지만 현실적으로 모든 $z$에 대해서 $p_{\theta}(z)p_{\theta}(x \vert z)$를 계산해서 더하는 것은 너무 많은 비용이 듭니다. 사후 확률 $p_{\theta}(z \vert x)=p_{\theta}(x \vert z)p_{\theta}(z)/p_{\theta}(x)$도 계산하기 어렵습니다. 이러한 문제를 해결하기 위해 VAE는 `variational inference` 방법을 사용합니다. `variational inference`는 사후 확률 분포를 직접 계산하는 대신, 더 간단한 형태의 분포로 사후 확률을 근사시키는 방법입니다. 즉, **새로운 함수 $q_{\phi}(z \vert x)$를 도입해 $p_{\theta}(z \vert x)$에 근사**시켜 문제를 해결하는 것입니다. 여기에서 $q_{\phi}(z \vert x)$가 바로 **encoder**입니다. 그리고 latent vector를 $z$로부터 데이터를 재구성하는 $p_\theta(x \vert z)$가 **decoder**입니다.

![](/assets/img/gan/vae_architecture.png)

encoder와 decoder가 잘 학습되기 위해서는 모델이 실제 데이터 분포를 얼마나 잘 설명하는지를 수치로 평가할 방법이 필요합니다. 그 척도가 바로 $\log p_\theta(x)$이지만, 앞서 설명했듯이 이를 계산하는 것은 사실상 불가능합니다. 따라서 **$\log p_{\theta}(x)$의 lower bound를 최대화**하는 방식으로 우회합니다. 이 lower bound를 ELBO(Evidence of Lower Bound)라고 부릅니다. jensen 부등식 $\mathbb{E}[f(X)] \ge f(\mathbb{E}[X])$에 의해 $\log p_{\theta}(x)$의 ELBO를 아래와 같이 구할 수 있습니다. 

$$\begin{align*}\log p_{\theta}(x) &= \log \int p_{\theta}(x,z) dz \\
&= \log \int q_{\phi}(z \vert x) \frac{p_{\theta}(x,z)}{q_{\phi}(z \vert x)} dz \ge \int q_{\phi}(z \vert x) \log\frac{p_{\theta}(x,z)}{q_{\phi}(z \vert x)} dz \end{align*}$$

$$\begin{align*}\int q_{\phi}(z \vert x) \log\frac{p_{\theta}(x,z)}{q_{\phi}(z \vert x)} &= \int q_{\phi}(z \vert x) \log p_{\theta}(x,z) - q_{\phi}(z \vert x) \log q_{\phi}(z \vert x) dz \\
&= \int q_{\phi}(z \vert x) \log p_{\theta}(x \vert z)p_{\theta}(z) - q_{\phi}(z \vert x) \log q_{\phi}(z \vert x) dz \\
&= \int q_{\phi}(z \vert x) \{\log p_{\theta}(x \vert z)+\log p_{\theta}(z)\} - q_{\phi}(z \vert x) \log q_{\phi}(z \vert x) dz \\
&= \int q_{\phi}(z \vert x)\log p_{\theta}(x \vert z) + q_{\phi}(z \vert x)\log p_{\theta}(z) - q_{\phi}(z \vert x) \log q_{\phi}(z \vert x) dz \\
&= \int q_{\phi}(z \vert x)\log p_{\theta}(x \vert z) - q_{\phi}(z \vert x)\log \frac{q_{\phi}(z \vert x)}{p_{\theta}(z)} dz \\
&= E_{z \sim q_{\phi}(z|x)} \left[ \log p_{\theta}(x|z) \right] - D_{KL}(q_{\phi}(z|x) \vert\vert p_{\theta}(z)) \\
\end{align*}$$
...
앞서 설명한 바와 같이, VAE는 실제 사후 분포 $p_{\theta}(z \vert x)$의 근사 분포 $q_{\phi}(z \vert x)$를 출력하는 encoder를 사용합니다. 실제 사후 분포 $p_{\theta}(z \vert x)$를 근사하는 것은 곧 $q_{\phi}(z \vert x)$가 $p_{\theta}(z \vert x)$에 최대한 가깝도록 만드는 것입니다. 따라서, 다음과 같이 두 분포 사이의 거리를 측정하는 [KL divergence](https://jieun121070.github.io/posts/%EB%B6%84%ED%8F%AC-%EA%B0%84%EC%9D%98-%EA%B1%B0%EB%A6%AC%EB%A5%BC-%EC%B8%A1%EC%A0%95%ED%95%98%EB%8A%94-%EB%B0%A9%EB%B2%95%EB%93%A4/)를 사용해 $$D_{KL}(q_{\phi}(z \vert x) \vert\vert p_{\theta}(z \vert x))$$를 최소화하는 문제로 바꿀 수 있습니다.
...
위 수식의 좌변은 VAE 학습 과정에서 최대화하고자 하는 것입니다. 관찰된 데이터 $x$가 모델에 의해 생성될 log likelihood $\log p_{\theta}(x)$는 최대화하고자 하고, $q_{\theta}(z \vert x)$와 $p_{\theta}(z \vert x)$ 사이의 거리 $$D_{KL}(q_{\theta}(z \vert x) \vert\vert p_{\theta}(z \vert x))$$는 최소화하고자 하기 때문입니다. 하지만 $p_{\theta}(z \vert x)$는 알 수 없으므로 $$D_{KL}(q_{\theta}(z \vert x) \vert\vert p_{\theta}(z \vert x))$$는 계산할 수 없습니다. 따라서 우변에 음수를 취해 loss function으로 사용합니다.

$$\begin{align*}L_{VAE}(\theta, \phi) = -\mathbb{E}_{z \sim q_{\phi}(z|x)}[\log p_{\theta}(x|z)] + D_{KL}(q_{\phi}(z|x) \vert\vert p(z))\end{align*}$$
