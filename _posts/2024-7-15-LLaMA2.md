...
- `Reinforcement Learning from Human Feedback (RLHF)`: 보상 모델이 예측한 점수 $r$를 사용해서 정책을 최적화합니다. 아래 loss function는 **Proximal Policy Optimization (PPO)** 알고리즘을 간단히 나타낸 것입니다. $R(g \vert p)$는 프롬프트 $p$에 대해 모델이 만든 응답 $g$에 부여되는 점수입니다. 여기에서 $\pi_\theta$는 현재 정책, $\pi_0$는 레퍼런스 정책을 나타내고, $\tilde{R}_c(g \vert p)$는 보상 모델이 매긴 점수입니다. 이를 통해 사람처럼 자연스러운 대화 스타일을 만듭니다.
